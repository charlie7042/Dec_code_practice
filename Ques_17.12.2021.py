...
# 2. It's not necessary that user will write full/exact product nameApple
#   - eg : "JBL Headphones bluetooth"
#   - I/P : headphones
item=0
toSearch = input("Search Here...").lower()
print()
searchlist=["p_name","brand","Category"]

for i in range(len(products)):
    for j in searchlist:
        word=products[i][j].lower()
        count=word.find(toSearch)
        if count==0:
            ...
            break
if item==0:
    print(f"Did not found your product : {toSearch}")
else:
    print(f"We found {item} product")
